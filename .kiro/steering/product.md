...
**Popup dashboard** — three-tab UI (320px wide):
...
- **Account tab**: email/password login form (signed-out state) or user card + sign-out button (signed-in state). Auth calls go directly to Supabase Auth REST API (`/auth/v1/token?grant_type=password`). OAuth sign-in via Google and GitHub using `chrome.identity.launchWebAuthFlow` — tokens parsed from redirect URL fragment. Session stored in `chrome.storage.local`.
