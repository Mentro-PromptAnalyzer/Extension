...
- **Rigor** — maps to `criticalThinking`: does it probe edge cases, risks, or alternatives?
...
A floating badge (score circle) appears next to the input bar. Hovering fans out four sub-bubbles with SVG arc rings showing per-dimension scores. The popup shows the latest score when the extension icon is clicked.
