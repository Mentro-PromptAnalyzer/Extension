...
**Popup dashboard** — two-tab UI (320px wide), Account tab is first and default. Settings tab is hidden when the user is signed out — only the Account tab is visible when unauthenticated:
- **Account tab** (default): signed-out state shows an OAuth-only welcome screen ("Welcome to Mentro" heading + description + Google/GitHub "Continue with" buttons — no email/password form). Signed-in state shows a clickable user card (avatar + email + caret) that expands an account-options dropdown. The dropdown displays linked-provider indicators (provider name + checkmark icon for already-linked providers, "Link [Provider]" buttons for unlinked ones), a divider, and a sign-out button — sign-out lives exclusively inside this dropdown menu, not as a standalone element. Provider linking reuses the OAuth flow; Supabase auto-links identities sharing the same email. Below the user card: borderless stat rows (label + large value + caret chevron, hover tint + caret slide animation). Stat rows: Prompts scored, Avg score, Top intent, Top platform. Clicking a row drills into a detail panel: Prompts (word count distribution — Short/Medium/Long CSS bars from `extension_prompts.word_count`), Avg Score (score band distribution — excellent/good/needs work), Top Intent (per-intent counts and percentages), Top Platform (per-platform prompt count + avg score). Back button returns to the stat list. Stats fetched from `extension_prompts` table via Supabase REST; polled every 10 s while popup is open.
- **Settings tab**: pills toggle, badge toggle, detailed metrics sub-toggle (nested under badge, disabled when badge is off — controls the 4 scoring dimension bubbles on badge hover, with an expandable inline "?" section showing colour-coded metric definitions), stat collection toggle (visible only when signed in), theme picker (6 themes: Dark, Midnight, Emerald, Rosé, Sunset, Nord — each with a unique brand color and background) — persisted via `chrome.storage.sync` and broadcast live to content scripts. Theme changes apply to both the popup (CSS custom properties via `[data-theme]` attribute) and the content-script overlays (dynamic brand RGB via `setThemeBrand()`). Reset to defaults button. When signed in, also shows a "Delete my data" button with a two-tap confirmation flow that permanently deletes all the user's prompt stats.
