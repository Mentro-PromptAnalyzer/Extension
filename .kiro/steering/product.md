...
- **Settings tab**: pills toggle, badge toggle, stat collection toggle — persisted via `chrome.storage.sync` and broadcast live to content scripts. Reset to defaults button. When signed in, also shows a "Delete my data" button with a two-tap confirmation flow that permanently deletes all the user's prompt stats.
