...
| `ollama.ts` | Thin proxy — `scoreWithOllama(text, heuristic?)` sends an `OLLAMA_SCORE` message (with optional `HeuristicContext`) to the background worker and returns the result. `HeuristicContext` carries pre-computed intent, flags, dimension scores, and TF-IDF topics so the background can inject them into the Ollama prompt. Does not fetch directly (content scripts on https:// pages cannot make http:// requests — Chrome mixed-content block) |

### `src/content/` — Injected into AI platform pages

| File | Responsibility |
|---|---|
| `index.ts` | Entry point — detects platform, polls for input element, attaches observers via `attachToInput()`, debounces analysis, sends messages to background. Tracks `activeInput`/`activeObserver` module-level refs; 1 s heartbeat re-attaches if SPA replaces the element; `visibilitychange` re-scores on tab focus. Two-layer scoring: heuristic fires at 300 ms debounce, Ollama async re-score fires at 1500 ms debounce with spinner feedback. `lastScoredText` tracks the last fully-scored text so observer/input/keyup event bursts from a single keystroke don't incorrectly hide pills — `hideFeedback()` only fires when the text has actually changed from `lastScoredText`. Before calling `scoreWithOllama`, builds a `HeuristicContext` (intent, flags, dimension scores, top TF-IDF topics) from the heuristic result and passes it so Ollama receives pre-digested signal rather than re-deriving it from scratch. |
...
- **Ollama system prompt** uses anchored scale descriptions (0/40/70/100 examples per dimension) and a calibration example with expected scores to keep scoring consistent across model runs. Suggestions rules explicitly require: referencing the actual subject matter of the prompt, being a concrete question the user could literally add to their prompt, targeting a different weak dimension each, with good/bad examples to calibrate the model away from generic advice. A **pre-analysis block** derived from `HeuristicContext` is injected before the prompt text, providing detected intent, key TF-IDF topics, heuristic scores, weakest dimensions, and detected signals — so Ollama spends tokens on grounded suggestions rather than re-scoring.
