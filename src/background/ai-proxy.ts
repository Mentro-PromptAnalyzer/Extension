...
export const SYSTEM_PROMPT = `You are an expert at evaluating the quality of prompts sent to AI assistants.
...
// ---------------------------------------------------------------------------
// Pre-analysis builder
// ---------------------------------------------------------------------------

export function buildPreAnalysis(heuristic: HeuristicContext): string {
...
export async function fetchAIScore(
  ...
  try {
    ...
    return { ownership, depth, critical, clarity, overall, intent, suggestions };
  } catch (err) {
